<?php
	...
	if(isset($_POST['submit-vehicle'])) {
		...
		$eid;
		$q1=mysqli_query($con, "select eid FROM register where cno='$ownercontno'");
			 $ret=mysqli_fetch_array($q1);
...
//die($eid);
//die($eid.$arkingnumber.$catename.$vehcomp.$vehreno.$ownername.
			//$ownercontno);

		$query=mysqli_query($con, "INSERT into vehicle_info(eid,ParkingNumber,VehicleCategory,VehicleCompanyname,RegistrationNumber,OwnerName,OwnerContactNumber) values('$eid','$parkingnumber','$catename','$vehcomp','$vehreno','$ownername','$ownercontno')");
		if ($query) {
